...
- `codex-reset-wait until <T> · since <T0> · attempt <N>` — one durable task spanning a whole Codex usage-window park (created/updated by `swarm:independent-review-loop`): `since <T0>` is set once, `until <T>` and `attempt <N>` are updated on each re-park; its description carries the hold / resume-in-place / attempt-bound / escalate rules the pulse follows. Closed only when Codex clears or the escalation resolves — never on bare resume.
...
- When an explicit shutdown request has been received, resolve the pulse job ID (CronList) and delete it (CronDelete) after the team has been shut down (idempotent — if the terminal step already deleted it, no matching job remains and that is fine)
