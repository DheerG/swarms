...
- If the target file's `autoMode.environment` already contains one or more entries with `<` placeholders → tell the user (naming the count if more than one), then ask via AskUserQuestion whether to replace the placeholder entry/entries with the constructed string. If "yes" → remove every entry containing `<` from the array and append the constructed string once (collapsing multiple placeholder entries to a single real entry); finish (skip the remaining merge bullets). If "no" → leave the placeholder entries in place and fall through to the exact-token append rule below; if that rule then skips on a same-host match, do nothing further.
