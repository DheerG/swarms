...
- **Write run-state before each phase.** Before entering any phase (Research → Deliver), write `~/.claude/teams/<team-name>/state.md` with `team`, `phase`, `approved-plan` (paste the facilitator's CONVERGED synthesis verbatim; blank before Approve), `last-signal-received` (blank, "CONVERGED", or "CONFIDENCE REACHED"; update when receiving a facilitator signal, before acting on it), `members-pending-review`, `progress` (mid-phase working state — "branch created", "files modified", "implementation complete, signal pending"; update at natural Execute checkpoints), and up to 5 recent `DECIDED:` items (include post-Approve user instructions as DECIDED items — e.g., `DECIDED: use JWT not OAuth (user, mid-Execute)`). Substitute `<team-name>` with the actual team name; run `mkdir -p ~/.claude/teams/<team-name>` via Bash before the first write. This is the recovery anchor if context compacts.
- **Read run-state on every pulse.** Before acting on a pulse, read `~/.claude/teams/<team-name>/state.md` to re-anchor on phase, plan, and decisions. Substitute `<team-name>` with the actual team name. Context may have compacted; the file has not.
- **Session-start self-check.** At the start of each session turn, if `~/.claude/teams/<team-name>/state.md` exists and phase is not Research, read it to reorient and run CronList to check pulse registration; re-register via CronCreate if absent.
...
After spawning all team members, create a heartbeat that prevents the lead from stalling. Substitute `<team-name>` in the prompt below with the actual team name from Step 8a before passing to CronCreate. Use **CronCreate** with:
- **cron**: `2,6,10,14,18,22,26,30,34,38,42,46,50,54,58 * * * *` (every 4 minutes, offset from round marks to avoid cache-miss alignment)
- **prompt**: "Pulse: read `~/.claude/teams/<team-name>/state.md` before acting — context may have compacted since the last write. If the file content surprises you (phase differs from your recollection, or decisions you don't remember), say 'Reorienting after compaction' once in your next user-facing message before proceeding. If you write state during this pulse turn and that write followed the re-read above, send one user-facing message: 'State anchored: [phase] / [progress].' Then check your state. If awaiting a facilitator signal (RESEARCH COMPLETE, CONVERGED, or CONFIDENCE REACHED) or user approval: check whether you have already waited for one pulse cycle. If this is the first pulse while waiting, continue waiting. If you have been waiting since the previous pulse, send a direct message to the facilitator naming the specific signal you are waiting for and asking them to evaluate whether conditions are met and send it. If you asked the user a question, evaluate whether you genuinely need their answer to proceed — if not, continue without it. If idle with no pending decisions, advance to your next phase. Only wait when you need a decision not covered by the approved plan. Do not narrate or acknowledge this pulse unless the conditions above require it (reorienting after compaction, or sending the 'State anchored' signal after a read-then-write cycle)."
- **recurring**: true
- **durable**: false
