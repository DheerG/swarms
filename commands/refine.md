...
5. **Phase arc (replaces launch.md Step 8f).**
   ...
   - **Deliver.** Follows the Deliver phase definition returned by `swarm:code-mode` at Step 8b verbatim with two overrides specific to this command: (a) do not create a new feature branch — the work is already on the current branch; (b) if a PR already exists for this branch, do not open a duplicate — push commits to the existing PR. If no PR exists and the ship definition calls for one, follow the file-based PR body pattern from the Deliver phase definition.
