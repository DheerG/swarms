...
**Extension modes.** If the invoked mode skill's frontmatter declares `extends:` naming a base (e.g., `extends: swarm:code-mode`, `extends: swarm:writing-mode`, or `extends: swarm:general-mode`), it is an extension mode. Read the frontmatter directly from the file at `.claude/skills/<name>/SKILL.md` to detect this — do not infer from body prose. Invoke the named base mode skill via the Skill tool **immediately after** the extension. The base provides Lead Identity, Facilitator Title, Facilitator Identity, base Mode-Specific Rules, base Suggest-Members Guidance, and the Phase Arc. The extension supplies **additive** Mode-Specific Rules and Suggest-Members Guidance on top of the base — treat them as supplementary, not replacing.
...
- If an extension appears to violate this contract (e.g., redefines phase semantics), treat it as malformed and surface the issue to the user before proceeding.
...
1. **Mode spec.** Invoke the mode skill via the Skill tool. If it is an extension mode, also invoke its declared base (see "Extension modes" above) and combine: use the base's Lead Identity, Facilitator, Phase Arc, and base rules; append the extension's additive Mode-Specific Rules and Suggest-Members Guidance supplements. If the mode (or its base) defines an outcomes question, use it. Otherwise fall back to "What are you working on?" If the mode (or its base) includes Pre-flight Reads, read those files before spawning.
