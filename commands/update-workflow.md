...
3. `## Settings` section (consumer-owned, preserved as-is — with one mechanical migration: replace the stale phrase `tier picked at setup` with `tier picked at the launch gate`, since the regenerated Workflow has no setup step and the tier now rides the Launch options. This migration appears in the Step 5 diff like any other change, so the user confirms it).
