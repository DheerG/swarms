...
3. **Team configuration.** Configure silently — do not ask a question per setting: invoke `[name]-mode` via the Skill tool (unqualified name) — this is your mode skill; invoke `swarm:suggest-members`, passing the mode skill's Suggest-Members Guidance and the confirmed outcomes; no lead research. Proceed to step 4 in the same response.
