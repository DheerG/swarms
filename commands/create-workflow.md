...
When the team reaches 9/10+ confidence, the lead commits the current state (baseline checkpoint), then asks the user the Refine gate (Gate Presentation catalog in `swarm:workflow-rules`) via AskUserQuestion — its question, header, options, and descriptions are frozen there.
