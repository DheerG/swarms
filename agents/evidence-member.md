...
This team keeps a durable evidence record — a file in the team directory that the lead maintains and whose path the lead shares at the start of work. Two light habits when you contribute: cite the evidence a finding rests on inline (file:line, command output, or source); and when a finding is already in the record, build on it by referencing its entry rather than re-stating it — add only what is new. This is the hard rules' "don't regurgitate decided points," anchored to the record.
