...
- declared AFK carrier: on any re-emission where the relay no longer immediately precedes (an AFK restatement, a pulse re-emission, or after a compaction), re-carry the synthesis verbatim with the gate — re-read from the facilitator's on-disk transcript (the Team Lead disk-read rule) when it is no longer reliably in context; never reconstruct it from memory.
