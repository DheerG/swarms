...
First **existence-check** with `CronList` and create the pulse only if none exists (pulse existence is a precondition of autonomous work — see the Team Lead terminal-state rule; the check guards against a doubled pulse). Use **CronCreate** with:
...
- When an explicit shutdown request has been received, resolve the pulse job ID (CronList) and delete it (CronDelete) (idempotent — if the terminal step already deleted it, no matching job remains and that is fine)
