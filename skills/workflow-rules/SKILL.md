...
First **existence-check** with `CronList` for the pulse signature and create the pulse only if no cron matches it — match the signature, not "any cron exists," so the user's own `/loop` or `/schedule` crons never suppress pulse creation. (Pulse existence is a precondition of autonomous work — see the Team Lead terminal-state rule.) Use **CronCreate** with:
