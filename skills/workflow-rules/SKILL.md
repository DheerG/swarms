...
- **Setup confirmation is mandatory on every launch.** Render the full setup confirmation summary as formatted markdown in the main window (the modal's question field renders plain text only; the main window renders markdown and stays in scrollback), carry a self-sufficient plain-text digest — enough to decide without scrolling up, never the full markdown block — inside the Launch ask's question text, and receive an explicit Launch response ("Launch the team", or a tier-carrying Launch pick from a not-yet-regenerated shortcut) — via AskUserQuestion, or (if that modal AFK-timed-out) the user's explicit typed answer to its durable plain-text restatement — before creating the team; no path exempts you, including outcomes passed inline with the command.
