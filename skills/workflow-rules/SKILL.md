...
- **Setup confirmation is mandatory on every launch.** Carry the full setup confirmation summary as formatted markdown in the `preview` of each Launch-ask option (previews render markdown in a side-by-side pane when an option is focused; the question field renders plain text only), carry a self-sufficient plain-text digest — enough to decide without relying on the preview pane, never the full markdown block — inside the Launch ask's question text, and receive an explicit Launch response ("Launch the team", or a tier-carrying Launch pick from a not-yet-regenerated shortcut) — via AskUserQuestion, or (if that modal AFK-timed-out) the user's explicit typed answer to its durable plain-text restatement — before creating the team; no path exempts you, including outcomes passed inline with the command.
