...
- **Probe before scoring at each rung.** During recursive refinement, the facilitator (and lead) must ask each reviewer "what is still missing?" before accepting their score. Convergence without the probe is premature exit. A "nothing remains" answer is not clearance to skip the rung — apply the mandatory-to-10 rule.
...
- When the lead signals implementation is complete, solicit a review and confidence score from each non-lead, non-facilitator team member individually. Probe each reviewer with "what is still missing?" before accepting their score. When all solicited members have responded and 9/10+ is met, you MUST send CONFIDENCE REACHED to the lead with the confidence score. 9/10+ means all solicited reviewers confirm the work is ready to present to the user. The probe applies at every rung in recursive refinement.
