...
Relay the facilitator's CONVERGED synthesis verbatim to the user. Do not re-derive or paraphrase. Then render the **Approve gate** from the Gate Presentation catalog in `swarm:workflow-rules` (subject: approach) — the synthesis rides verbatim in each option's `preview` so it is readable inside the modal; question, header, options, and descriptions are frozen in the catalog.
...
When the team reaches 9/10+ confidence, the lead commits the current state (`checkpoint: rung 9 — <summary>`), then presents the **unified pre-ship gate** — the **Finish gate** in the Gate Presentation catalog in `swarm:workflow-rules` — via AskUserQuestion. Its question, header, and four thoroughness-descending options ("Recursive refinement + independent review (Recommended)", "Recursive refinement only", "Independent review loop only", "Ship as is") are frozen there; render them verbatim.
