...
**Use file-based input for commit messages and PR bodies.** Write the message/body to `/tmp/swarm-commit-msg.md` or `/tmp/swarm-pr-body.md` (overwrite if it exists — never reuse a stale file), then pass with `git commit -F <file>` or `gh pr create --body-file <file>`, then delete the file. Inline HEREDOC + command substitution (`-m "$(cat <<EOF ...)"`, `--body "$(cat <<EOF ...)"`) triggers the bash safety heuristic and prompts unconditionally in auto mode — file-based input bypasses it.
