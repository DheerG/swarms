...
3. **Terminal.** Once the work is shipped and no run-state task remains open, end the run cleanly per the terminal-state Team Lead rule (Step 1): at this true end — never at PR-creation — find the pulse by its signature (launch.md Step 8e) via `CronList` and delete it (`CronDelete`) FIRST — single match → delete, zero → already gone, multiple → surface, never delete on ambiguity — then ask the user via AskUserQuestion whether to keep the team open or shut down. Match by signature rather than a recalled ID (gone after a compaction); deletion is what stops the pulse churn, the question is UX, not the burn-control.
