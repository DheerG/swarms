...
3. **Terminal.** Once the work is shipped and no run-state task remains open, end the run cleanly per the terminal-state Team Lead rule (Step 1): at this true end — never at PR-creation — resolve the pulse job ID (`CronList`) and delete it (`CronDelete`) FIRST, then ask the user via AskUserQuestion whether to keep the team open or shut down. Resolve the ID from `CronList` rather than from memory (a recalled ID may be gone after a compaction); deletion is what stops the pulse churn, the question is UX, not the burn-control.
