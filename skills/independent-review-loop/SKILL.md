...
Resolve the diff base once: on the common path a PR exists, so the base is the PR base (`gh pr view --json baseRefName`); if no PR was created (a commit-only / push-only ship), fall back to the repo default branch (`git symbolic-ref refs/remotes/origin/HEAD --short` stripped of `origin/`), and if that is unset, ask the user for the base or skip the loop — never error. Then each round:
...
5. **Commit AND push this round's fixes, then re-run.** The review reads the *committed* `git diff <base>...HEAD`, so the lead must commit each round's fixes (e.g., `review: round N — <summary>`) and push them — to the PR if one exists, otherwise the branch — before re-running. Otherwise the reviewer re-reads stale committed state, the loop never converges, and every just-fixed finding reappears unchanged and is misread as oscillation. Pushing each round also keeps the remote reflecting the work as it hardens. After committing and pushing, re-run the review on the whole diff.

**Termination (engine-neutral — identical on both paths)** — stop ONLY when a whole-PR review returns an explicit, format-conformant **"no in-scope functional findings"** result (a positive no-findings statement within a well-formed review, after the lead's triage). A clean round must be that affirmative statement, never the *absence* of one: empty output, a truncated or off-format response, a clarifying question, or a non-zero exit is a **FAILED review, not a clean one** — retry once, then degrade (Codex: take the error path / offer the Swarm fallback; Swarm: re-spawn the reviewer). Never read silence or a non-answer as "clean": a false "clean → done" silently ships bugs while reporting success, defeating the loop's whole purpose. (This is a single validity check — a clean round is a positive "no findings" statement — not a field-by-field parser.) Both Codex and the swarm-native reviewers emit the same finding format, so the terminator never depends on an engine-specific field; on the Codex path an `overall_correctness: patch is correct` verdict, when present, is a convenience corroborator only, not the stop signal. Expect this to take many rounds — 8–10 rounds of genuinely different findings is normal and runs fully autonomously; do not stop early because a round was quiet if the next whole-PR pass would surface more. Run the loop to its natural end.
...
>
> Prioritize each finding [P0]–[P3]. **No code suggestions** — for each finding, explain in prose what breaks, the exact conditions that trigger it, and the consequence. If you find no in-scope functional issues, **state that explicitly** (e.g., "No in-scope functional findings.") — do not return an empty or merely terse response.
