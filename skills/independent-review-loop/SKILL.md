...
Resolve the diff base once, matching what the user is actually shipping so the reviewer audits the same diff that will land: on the common path a PR exists, so the base is the PR base (`gh pr view --json baseRefName`). If no PR exists (a commit-only / push-only ship), use the base the **ship definition's** branch strategy implies (the branch the work was cut from, per `.claude/swarm-ship.md`); if that isn't determinable, fall back to the repo default branch (`git symbolic-ref refs/remotes/origin/HEAD --short` stripped of `origin/`); and if that too is unset, ask the user for the base or skip the loop — never error. Then each round:
