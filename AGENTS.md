...
- **In-modal render binding — carrier facts verified on v2.1.198 only; structure stable, numbers may drift.** The Step 4/Step 7 binding (roster/plan carried inside the AskUserQuestion question text, prose demoted to fallback) rests on the question field accepting long content (no schema cap; the TUI scrolls it) while option descriptions cap at ~1000 chars. Provenance: the earlier same-turn-prose binding was satisfied twice in a live run and the user still saw nothing — a focus-capturing modal renders over same-turn prose — so in-modal carry is what the user demonstrably read. Drift failure modes: a future hard cap on question text fails loud (tool-call reject; recover by moving overflow to the prose fallback); a soft render-clip fails silent (tail-truncation), which the front-load rule (Team + Cost tier first) exists to survive — front-loading is the user-seat graceful degradation for exactly this drift.
